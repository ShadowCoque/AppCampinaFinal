...
import { persistirFirma } from "../../data/archivos";
 ...
 * no puede entregar sola: una firma que desapareció del dispositivo, o un
 * trámite que el servidor dejó de conocer. Antes eso no se decía en ninguna
 * parte y el operador creía que todo había llegado.
 *
...
export function TarjetaEnvio({ situacion, onCambio, onDibujando }: Props) {
  ...
  const guardarYEnviar = async (rol: RolAdjunto, archivo: { uri: string }) => {
    ...
    <Card title="Envío al servidor del Club" icon="cloud-upload">
      {noExiste ? (
        ...
        <InfoNote tone="success" icon="cloud-done">
          El servidor tiene el trámite completo: sus datos y sus firmas.
        </InfoNote>
      ) : null}
